...
public class ManualConfig {
    ...
    private String selectedProvider = "ollama"; // New: internal provider selection
    ...
    // New getters and setters for Clawbot-MC
    public String getSelectedProvider() {
        return selectedProvider;
    }

    public void setSelectedProvider(String selectedProvider) {
        this.selectedProvider = selectedProvider != null ? selectedProvider : "ollama";
    }
    ...
    public void setOllamaModel(String ollamaModel) {
        this.ollamaModel = ollamaModel != null ? ollamaModel : "llama3.2";
    }
    ...
    public static String getActiveProvider() {
        try {
            ManualConfig config = AIPlayer.CONFIG;
            if (config != null && config.getSelectedProvider() != null && !config.getSelectedProvider().isEmpty()) {
                return config.getSelectedProvider();
            }
        } catch (Exception e) {
            LOGGER.debug("Could not get config, using fallback");
        }
        // Fallback to system properties for backward compatibility
