...
import net.luanvha2550_hash.PlayerUtils.ResourceEvaluator;
import net.luanvha2550_hash.PlayerUtils.ThreatDetector;
import net.luanvha2550_hash.PlayerUtils.ProjectileDefenseUtils;
import net.luanvha2550_hash.PlayerUtils.MobThreatEvaluator;
...
public class RLAgent {
    ...
    // Tracking for adaptive epsilon
    private static final double MAX_EPSILON = 0.3;
    private double recentAvgReward = 0;
    private int rewardSampleCount = 0;
    private static final int REWARD_SAMPLE_SIZE = 20;

    // Track recent rewards for adaptive epsilon
    private final java.util.ArrayDeque<Double> recentRewards = new java.util.ArrayDeque<>();

    // Update epsilon after each episode or iteration
    public void decayEpsilon() {
        // Calcula média móvel das recompensas recentes
        if (!recentRewards.isEmpty()) {
            recentAvgReward = recentRewards.stream()
        ...
        // Epsilon adaptativo:
        // Se recompensas estão altas (>10 média), explora menos
        // Se recompensas estão baixas, explora mais
        if (recentAvgReward > 10) {
            // Bom desempenho - explora menos
            epsilon = Math.max(MIN_EPSILON, epsilon * 0.95);
        } else if (recentAvgReward < 0) {
            // Desempenho ruim - explora mais para encontrar novas estratégias
            epsilon = Math.min(MAX_EPSILON, epsilon * 1.1);
        } else {
            // Decay normal
            epsilon = Math.max(MIN_EPSILON, epsilon * EPSILON_DECAY_RATE);
        }

        LOGGER.info("Epsilon adaptativo: {} (média recompensas: {:.1f})", epsilon, recentAvgReward);
    }

    /**
     * Registra uma recompensa para uso no epsilon adaptativo
     */
    public void recordReward(double reward) {
        recentRewards.add(reward);
        if (recentRewards.size() > REWARD_SAMPLE_SIZE) {
            recentRewards.pollFirst();
        }
    }
        ...
        for (Action action : possibleActions) {
            ...
            switch (action) {
                ...
                case SHOOT_ARROW:
                    ...
                    boolean hasBowOrCrossbow = net.luanvha2550_hash.PlayerUtils.RangedWeaponUtils.hasBowOrCrossbow(bot);
                    boolean hasArrows = net.luanvha2550_hash.PlayerUtils.RangedWeaponUtils.hasArrows(bot);
    ...
    public static double getEntityRisk(EntityDetails entity, int botX, int botY, int botZ) {
        ...
        if (net.luanvha2550_hash.Overlay.ThreatDebugManager.isDebugEnabled()) {
        ...
        if (net.luanvha2550_hash.Overlay.ThreatDebugManager.isDebugEnabled()) {
            ...
            net.luanvha2550_hash.Overlay.ThreatDebugManager.updateThreat(
                entityUUID,
                entity.getName(),
                baseThreat,
                distance,
    ...
    public void recordTransition(State fromState, State toState, Action action, double reward, double podValue) {
        // Registra recompensa para epsilon adaptativo
        recordReward(reward);
    ...
    public State getCurrentState(net.minecraft.server.network.ServerPlayerEntity bot) {
        return net.luanvha2550_hash.GameAI.BotEventHandler.createInitialState(bot);
    }
